...
/* Dedupe petugas berdasarkan userId, gabungkan subLabel (city/name) */
function groupPetugasByUserId(items: UserProgress[]) {
  const map = new Map<string, { userId: string; name: string; subLabels: Set<string> }>();
  for (const it of items) {
    const key = it.userId;
    const name = it.user?.name || it.userId;
    const sub = (it.district?.city?.trim() || "") || (it.district?.name?.trim() || "");
    if (!map.has(key)) map.set(key, { userId: key, name, subLabels: new Set() });
...
function SPJ() {
  ...
  const [filter, setFilter] = useState({ jenisSurvei: "", statusPengajuan: "" });

  // ====== STATE MASS INPUT ======
  type Row = { id: string; userId: string; verifyNote: string; submitting?: boolean; result?: "ok" | "err"; errMsg?: string };
  const [subSurveyActivityId, setSubSurveyActivityId] = useState("");
  const [file, setFile] = useState<File | null>(null);
  const [rows, setRows] = useState<Row[]>([{ id: uid(), userId: "", verifyNote: "" }]);

  // ====== MODAL DETAIL / UPDATE ======
  const [selectedSPJ, setSelectedSPJ] = useState<SPJWithUserNSubSurvey | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [update, setUpdate] = useState({ id: "", status: "Disetujui", verifyNote: "" });
  ...
  const { data: userProgressData, loading: loadingPetugas, refetch: refetchPetugas } = useQuery(
    GET_USER_PROGRESS_BY_SUBSURVEY_ID,
    { variables: { subSurveyActivityId }, skip: !subSurveyActivityId, fetchPolicy: "cache-and-network" }
  );
  ...
  const [updateStatus, { loading: newloading }] = useMutation(UPDATE_SPJ_STATUS, {
    refetchQueries: [{ query: GET_ALL_SPJ }],
  });

  // ====== OPTIONS ======
  const activityOptions = useMemo(
    () => (subSurveyData?.allSubSurveyActivities ?? []).map((s: { id: string; name: string }) => ({ value: s.id, label: s.name ?? "-" })),
    [subSurveyData]
  );

  const petugasOptions = useMemo(() => {
    const raw: UserProgress[] = userProgressData?.userProgressBySubSurveyActivityId ?? [];
  ...
  const filteredSPJ = useMemo(() => {
    ...
    return rows.filter((spj) => {
      ...
      const matchesJenis = !filter.jenisSurvei || spj.subSurveyActivity?.name === filter.jenisSurvei;
      const matchesPengajuan = !filter.statusPengajuan || spj.submitState === filter.statusPengajuan;
  ...
  // ====== HANDLERS ======
  const onAddRow = () => setRows(prev => [...prev, { id: uid(), userId: "", verifyNote: "" }]);
  const onRemoveRow = (id: string) => setRows(prev => prev.filter(r => r.id !== id));
  const onSetRow = (id: string, patch: Partial<Row>) => setRows(prev => prev.map(r => (r.id === id ? { ...r, ...patch } : r)));
  const onFileChange = (e: React.ChangeEvent<HTMLInputElement>) => setFile(e.target.files?.[0] || null);

  const isReady = !!subSurveyActivityId && !!file && rows.length > 0 && rows.every(r => !!r.userId);
  ...
  const submitAll = async (e: React.FormEvent) => {
    ...
    setRows(prev => prev.map(r => ({ ...r, submitting: true, result: undefined, errMsg: undefined })));
    ...
    for (const r of rows) {
      try {
        ...
        ok++; onSetRow(r.id, { submitting: false, result: "ok" });
      } catch (ex: any) {
        err++; onSetRow(r.id, { submitting: false, result: "err", errMsg: ex?.message || "Gagal" });
      }
    }
    ...
    if (ok && !err) {
      ...
      toast(`Sebagian berhasil: OK ${ok}, Gagal ${err}. Periksa baris gagal lalu kirim ulang.`, { icon: "⚠️" });
    } else {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      if (!update.id || !update.status) return toast.error("Semua field wajib diisi!");
      await updateStatus({ variables: { input: { id: update.id, status: update.status, verifyNote: update.verifyNote || undefined } } });
    ...
    <div className="max-w-screen-xl mx-auto px-3 sm:px-6 md:px-8 py-4 space-y-4 font-Poppins">
      ...
      <div className="bg-white rounded-lg p-4 shadow space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium mb-1">Jenis Survei</label>
            <HUComboBox
              value={filter.jenisSurvei || null}
              onValueChange={(v) =>
                setFilter((prev) => ({ ...prev, jenisSurvei: (v ?? "") as string }))
              }
              options={[
          ...
          <div>
            ...
            <HUComboBox
              ...
              onValueChange={(v) =>
                setFilter((prev) => ({ ...prev, statusPengajuan: (v ?? "") as string }))
              }
          ...
          <tbody className="bg-white">
            ...
            {filteredSPJ.map((spj) => (
              <tr key={spj.id} className="border-t">
                <td className="px-6 py-4 font-semibold text-gray-900">{spj.user?.name || "-"}</td>
                <td className="px-6 py-4">{spj.subSurveyActivity?.name || "-"}</td>
      ...
      <div className="sm:hidden space-y-3">
        {!SPJData ? (
          <div className="bg-white rounded-lg p-3 shadow text-center">Memuat data…</div>
            ...
            <div key={spj.id} className="bg-white rounded-lg p-3 shadow">
              <div className="flex justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-gray-600">{spj.subSurveyActivity?.name || "-"}</p>
                </div>
                <button
                  onClick={() => {
                    setSelectedSPJ(spj);
      ...
      {isModalOpen && selectedSPJ && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center overflow-y-auto py-20" role="dialog" aria-modal="true">
          <div className="bg-white w-[92%] sm:w-[560px] rounded-lg shadow-lg max-h-[90vh] flex flex-col">
            <div className="flex justify-between items-center px-4 sm:px-6 py-4 border-b">
              ...
              {selectedSPJ.eviDocumentSignedUrl ? (
                <a target="_blank" href={selectedSPJ.eviDocumentSignedUrl || ""} className="bg-blue-600 text-white px-3 py-2 rounded-md text-sm">
                  Lihat Bukti
                </a>
              ) : <span className="text-xs text-gray-500">(Bukti belum tersedia)</span>}
            </div>

            <div className="p-4 sm:p-6 space-y-3 overflow-y-auto text-sm sm:text-base">
              <p><strong>Nama Petugas:</strong> {selectedSPJ.user?.name || "-"}</p>
              <p><strong>Jenis Survei:</strong> {selectedSPJ.subSurveyActivity?.name || "-"}</p>
              <p><strong>Submit State:</strong> {selectedSPJ.submitState}</p>
              <p><strong>Submit Date:</strong> {selectedSPJ.submitDate || "-"}</p>
              <p><strong>Approve Date:</strong> {selectedSPJ.approveDate || "-"}</p>
              <p><strong>Catatan:</strong> {selectedSPJ.verifyNote || "-"}</p>

              {user?.role === "Keuangan" || user?.role === "Superadmin" ? (
                <form onSubmit={handleUpdate} className="space-y-3 pt-4 border-t mt-2">
                  <input type="hidden" id="id" value={update.id} onChange={handleUpdateChange} />
                  <div>
                    <label className="block text-sm font-medium mb-1">Ubah Status</label>
                    <HUComboBox
                      value={update.status || null}
                      onValueChange={(v) => setUpdate((prev) => ({ ...prev, status: (v ?? "") as string, id: selectedSPJ.id }))}
                  ...
                  <div>
                    <input
                      ...
                      onChange={(e) => setUpdate({ ...update, verifyNote: e.target.value, id: selectedSPJ.id })}
                      className="w-full px-3 py-2 border rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div className="flex flex-col sm:flex-row sm:justify-between gap-2 pt-2">
                    <button type="submit" disabled={newloading} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">
                      {newloading ? "Menyimpan..." : "Update Status"}
                    </button>
                    <button
                      onClick={() => { setIsModalOpen(false); setSelectedSPJ(null); setUpdate({ id: "", status: "Disetujui", verifyNote: "" }); }}
                ...
                <button
                  onClick={() => { setIsModalOpen(false); setSelectedSPJ(null); }}
      ...
      {(user?.role === "Admin" || user?.role === "Superadmin") && (
        <div className="bg-white rounded-lg p-4 shadow space-y-3">
          ...
          <div className="grid md:grid-cols-[1fr_auto] gap-3">
            <div>
              <label className="block text-sm font-semibold mb-1">Bukti Pengajuan Honor</label>
              <input
                type="file"
                accept=".pdf,image/*"
              ...
              />
              <p className="text-xs text-gray-500 mt-1">Format: PDF/JPG/PNG. Sesuaikan dengan kebutuhan (Maks. 5MB)</p>
            </div>
                ...
                {rows.map((r, idx) => (
                  <tr key={r.id} className="border-t pb-52">
                    ...
                    <td className="px-3 py-2 min-w-[260px]">
                      <HUComboBox
                        value={r.userId || null}
                        onValueChange={(v) => onSetRow(r.id, { userId: (v ?? "") as string })}
                        options={
                          loadingPetugas
                            ? []
                            : petugasOptions.map(opt => ({
                                ...opt,
                                disabled: !!opt.value && rows.some(rr => rr.id !== r.id && rr.userId === opt.value),
                              }))
                        }
                        placeholder={
                          !subSurveyActivityId
                    ...
                    <td className="px-3 py-2">
                      <input
                        value={r.verifyNote}
                        onChange={(e) => onSetRow(r.id, { verifyNote: e.target.value })}
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      {r.submitting ? (
                        <span className="text-xs text-gray-500">Mengirim…</span>
                      ) : r.result === "ok" ? (
                        <span className="text-xs text-green-700">OK</span>
                      ) : r.result === "err" ? (
                        <span className="text-xs text-red-600" title={r.errMsg}>Gagal</span>
                      ) : (
                        <button
                          type="button"
          ...
          <div className="flex justify-end">
            <button
              onClick={submitAll}
              disabled={!subSurveyActivityId || !file || rows.some(r => !r.userId)}
